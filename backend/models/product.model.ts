...
const PRODUCT_FIELDS = 'p.id, p.unique_id, p.name, p.image, p.price, p.created_at, c.name AS category_name, c.id AS category_id';

// Create a new product
export const createProduct = async (name: string, image: string, price: number, category_id: number) => {
    const text = 'INSERT INTO products(name, image, price, category_id) VALUES($1, $2, $3, $4) RETURNING id, unique_id';
...
// Update a product
export const updateProduct = async (id: number, name: string, image: string, price: number, category_id: number) => {
    const text = `
        UPDATE products 
